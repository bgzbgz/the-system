...
function generateStageInsights(logs: FactoryLog[]): Array<{
  ...
  const stageConfig: Record<string, { name: string; order: number }> = {
    ...
    'example-gen': { name: 'Example Generator', order: 5 },
    'copy-write': { name: 'Copy Writer', order: 6 },
    'template-select': { name: 'Template Selection', order: 7 },
    'tool-build': { name: 'Tool Builder', order: 8 },
    'brand-audit': { name: 'Brand Guardian', order: 9 },
    'qa-eval': { name: 'QA Evaluation', order: 10 },
    'feedback-apply': { name: 'Feedback Applied', order: 11 },
  };
